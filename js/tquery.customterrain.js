tQuery.registerStatic('createCustomTerrain', function (opts) {
    return new tQuery.CustomTerrain(opts);
});
...
tQuery.registerStatic('CustomTerrain', function (opts) {
    ...
    this._changed = true;
    this._geometry = tGeometry;
    this._texture = texture;
    this._materials = new THREE.MeshBasicMaterial({ map: this._texture });
});

// make it pluginable
tQuery.pluginsInstanceOn(tQuery.CustomTerrain);

tQuery.CustomTerrain.prototype.getVertice = function (x, z) {
    for (var i = 0; i < this._geometry.vertices.length; i++) {
        if (this._geometry.vertices[i].x == x && this._geometry.vertices[i].z == z) {
            return this._geometry.vertices[i];
        }
    }    
}

tQuery.CustomTerrain.prototype.getHeightAt = function (x, z) {
    return this.getVertice(x, z).y;
}

tQuery.CustomTerrain.prototype.getTexture = function () {
    return this._texture;
}

tQuery.CustomTerrain.prototype.getGeometry = function () {
    return tQuery(this._geometry);
}

tQuery.CustomTerrain.prototype.getMesh = function () {
    if (this._changed) {
...
tQuery.CustomTerrain.prototype.setGeometry = function (geometry) {
    this._changed = true;
